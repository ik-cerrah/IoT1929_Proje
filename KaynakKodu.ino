#include <pitches.h>
#include <Tone32.h>

#include <SPI.h> 
#include <MFRC522.h>
#include <Arduino.h>
MFRC522 rfid(21, 22); 
byte Kart[4] = {233,179,102,194}; 
boolean Yetki; 
int ringBuzzer=1;
int counter=0;
int hata_counter=0;
#define led1 32
#define led2 27

#define echo 12
#define trig 13 
#define BLYNK_PRINT Serial
#include <WiFi.h>
#include <WiFiClient.h>
#include <BlynkSimpleEsp32.h>
int pinValue;
int freq = 2000;
int channel = 0;
int resolution = 8;
int buzzerpin = 33; 
char auth[] = "E7Fty_fKSm-R6q3AVRieDOGHF2FGE9bY";
char ssid[] = "TurkTelekom_TA392";
char pass[] = "t73pxqGc";
#define  C0 16.35
#define Db0 17.32
#define D0  18.35
#define Eb0 19.45
#define E0  20.60
#define F0  21.83
#define Gb0 23.12
#define G0  24.50
#define Ab0 25.96
#define LA0 27.50
#define Bb0 29.14
#define B0  30.87
#define C1  32.70
#define Db1 34.65
#define D1  36.71
#define Eb1 38.89
#define E1  41.20
#define F1  43.65
#define Gb1 46.25
#define G1  49.00
#define Ab1 51.91
#define LA1 55.00
#define Bb1 58.27
#define B1  61.74
#define C2  65.41
#define Db2 69.30
#define D2  73.42
#define Eb2 77.78
#define E2  82.41
#define F2  87.31
#define Gb2 92.50
#define G2  98.00
#define Ab2 103.83
#define LA2 110.00
#define Bb2 116.54
#define B2  123.47
#define C3  130.81
#define Db3 138.59
#define D3  146.83
#define Eb3 155.56
#define E3  164.81
#define F3  174.61
#define Gb3 185.00
#define G3  196.00
#define Ab3 207.65
#define LA3 220.00
#define Bb3 233.08
#define B3  246.94
#define C4  261.63
#define Db4 277.18
#define D4  293.66
#define Eb4 311.13
#define E4  329.63
#define F4  349.23
#define Gb4 369.99
#define G4  392.00
#define Ab4 415.30
#define LA4 440.00
#define Bb4 466.16
#define B4  493.88
#define C5  523.25
#define Db5 554.37
#define D5  587.33
#define Eb5 622.25
#define E5  659.26
#define F5  698.46
#define Gb5 739.99
#define G5  783.99
#define Ab5 830.61
#define LA5 880.00
#define Bb5 932.33
#define B5  987.77
#define C6  1046.50
#define Db6 1108.73
#define D6  1174.66
#define Eb6 1244.51
#define E6  1318.51
#define F6  1396.91
#define Gb6 1479.98
#define G6  1567.98
#define Ab6 1661.22
#define LA6 1760.00
#define Bb6 1864.66
#define B6  1975.53
#define C7  2093.00
#define Db7 2217.46
#define D7  2349.32
#define Eb7 2489.02
#define E7  2637.02
#define F7  2793.83
#define Gb7 2959.96
#define G7  3135.96
#define Ab7 3322.44
#define LA7 3520.01
#define Bb7 3729.31
#define B7  3951.07
#define C8  4186.01
#define Db8 4434.92
#define D8  4698.64
#define Eb8 4978.03

#define BPM 120
#define H 2*Q 
#define Q 60000/BPM 
#define E Q/2  
#define S Q/4 
#define W 4*Q 

  void setup() 
{ 
  Serial.begin(9600);
  pinMode(echo,INPUT);
  pinMode(trig,OUTPUT); 
  
 
  
  ledcSetup(channel, freq, resolution);
  ledcAttachPin(buzzerpin, channel);
  SPI.begin();
  rfid.PCD_Init(); 
  Blynk.begin(auth, ssid, pass);
  
} 
int RfidOkuma(){
   Yetki = true;
  if ( rfid.PICC_IsNewCardPresent())
  { 
    if ( rfid.PICC_ReadCardSerial())
    { Serial.print("Okunan Kart ID: ");
     for(int a=0;a<4;a++){
      //Serial.print(rfid.uid.uidByte[a]);
      Serial.print(" ");  
      }
      Serial.print("\n"); 
    } 
    for (int i = 0; i < 4; i++) 
    {
      if (rfid.uid.uidByte[i] != Kart[i])
      {
        Yetki = false;
      }
    } 
   return Yetki;
  }
  }
 
void mytone(int freq, int duration) {
   int onay=RfidOkuma();
  Serial.println(onay);
  if(onay == 1)
  {
    Serial.println("Kart doğrulandı");
    Serial.println("Alarm kapandi");
    exit(0);
    }
   else{
  pinMode(led1,OUTPUT); 
  pinMode(led2,OUTPUT); 
  ledcWriteTone(channel, freq);
  
  if(counter % 2 == 0)
  {
   
    digitalWrite(led1,HIGH);
    delay(duration);
    digitalWrite(led1,LOW);
    
  }
  else
  {
   
    digitalWrite(led2,HIGH);
    delay(duration);
    digitalWrite(led2,LOW);
    
  }
  ledcWriteTone(channel, 0);
  counter++;
  }
 
  
  
}
 
 

BLYNK_WRITE(V1){
  pinValue = param.asInt();
  }
  
  
  
  

void loop()
{ 
  
  Blynk.run();
  
  
  int distance,theTime;
 while(pinValue == 1){
  int hata=RfidOkuma();
  if(hata == 0){
    hata_counter++;
    if(hata_counter%3 == 0 && hata_counter != 0){
      callAlarm();
          
      }
      delay(2000);
    }
    if(hata == 1){
      hata_counter == 0;
      Serial.println("Kart onaylandı");
      Serial.println("Hoşgeldiniz");
      pinValue = 0;
      
      }
  digitalWrite(trig,LOW);
  delayMicroseconds(2);
  digitalWrite(trig,HIGH);
  delayMicroseconds(10);
  digitalWrite(trig,LOW);
  theTime = pulseIn(echo,HIGH);
  distance = (theTime/2)/29.1;
 
  Serial.println(distance);
  if(distance <= 10 ){
    Blynk.notify("Hareket Algilandi!!!");
    callAlarm();
    }
    Blynk.run();
 }

}

  
  void callAlarm() { 
  
  while(ringBuzzer==1){
    mytone(LA3,Q); 
    mytone(LA3,Q);
    mytone(LA3,Q);
    mytone(F3,E+S);
    mytone(C4,S);

    mytone(LA3,Q);
    mytone(F3,E+S);
    mytone(C4,S);
    mytone(LA3,H);

    mytone(E4,Q); 
    mytone(E4,Q);
    mytone(E4,Q);
    mytone(F4,E+S);
    mytone(C4,S);

    mytone(Ab3,Q);
    mytone(F3,E+S);
    mytone(C4,S);
    mytone(LA3,H);

    mytone(LA4,Q);
    mytone(LA3,E+S);
    mytone(LA3,S);
    mytone(LA4,Q);
    mytone(Ab4,E+S);
    mytone(G4,S);

    mytone(Gb4,S);
    mytone(E4,S);
    mytone(F4,E);
    delay(E);//PAUSE
    mytone(Bb3,E);
    mytone(Eb4,Q);
    mytone(D4,E+S);
    mytone(Db4,S);

    mytone(C4,S);
    mytone(B3,S);
    mytone(C4,E);
    delay(E);//PAUSE QUASI FINE RIGA
    mytone(F3,E);
    mytone(Ab3,Q);
    mytone(F3,E+S);
    mytone(LA3,S);

    mytone(C4,Q);
    mytone(LA3,E+S);
    mytone(C4,S);
    mytone(E4,H);

    mytone(LA4,Q);
    mytone(LA3,E+S);
    mytone(LA3,S);
    mytone(LA4,Q);
    mytone(Ab4,E+S);
    mytone(G4,S);

    mytone(Gb4,S);
    mytone(E4,S);
    mytone(F4,E);
    delay(E);//PAUSE
    mytone(Bb3,E);
    mytone(Eb4,Q);
    mytone(D4,E+S);
    mytone(Db4,S);

    mytone(C4,S);
    mytone(B3,S);
    mytone(C4,E);
    delay(E);//PAUSE QUASI FINE RIGA
    mytone(F3,E);
    mytone(Ab3,Q);
    mytone(F3,E+S);
    mytone(C4,S);

    mytone(LA3,Q);
    mytone(F3,E+S);
    mytone(C4,S);
    mytone(LA3,H);

    delay(2*H);
 ringBuzzer=0;
  }

}
